...
title: "从 GPIO4 到板载 MIC 声谱图：ESP32-S3 驱动 25T 超声波的完整波形与观测模型"
description: "从 ESP32-S3 GPIO4 的载波方波、LEDC 占空比、Burst 包络和外部驱动，一路推导到 25T 压电换能器、Actions ATS3085S 板载 MIC、USB UAC PCM、FFT、声谱图、混叠、非线性和实验验证。"
authors: [w0x7ce]
tags: [嵌入式系統, 技術深度]
date: 2026-08-17
keywords:
...
当前测试固件的默认配置是：`GPIO4`、25 kHz 载波、50% 载波占空、`20 ms ON + 980 ms OFF`，也就是 1 Hz、2% 的包络。网页 `8766` 只控制 ESP32-S3 的串口参数；`8765` 是 Windows 宿主机上的分析页面，它读取 Actions ATS3085S 评估板通过 USB UAC 上传的板载 MIC PCM，再进行 FFT、声谱图和相对 dB 显示。它不是原始空气声压计，也不是换能器端电压波形查看器。
...
本次实验使用的是 ESP32-S3 N16R8 开发板、一个标称 25T 的压电超声换能器、外部驱动/限流级，以及一块带板载 MIC 模块的 Actions ATS3085S 评估板。ATS3085S 通过 USB UAC 向 Windows 提供麦克风 PCM，Windows 宿主机上的本地 `8765` 页面负责后续波形、FFT 和声谱图分析。
...
| Actions ATS3085S 板载 MIC 模块 | 板载麦克风及其前端把局部声学变化送入芯片音频采集链路 | 该板卡、安装位置、增益和频响条件下的接收变化 | 空气中绝对声压、25T 端电压/电流、完整超声频段响应 |
| Actions 芯片音频采集与 USB UAC | 按固件的采样、增益、滤波、缓冲和时序向主机输出 PCM；当前 demo 对外为 16 kHz / 16-bit / mono | 导出 PCM 的数值、时间块和设备端处理后的相对变化 | 原始 MIC 元件输出、芯片内部每一级时序、未经过处理的 ADC 波形 |
| 8765 Windows 分析页 | 对收到的 PCM 做 RMS/Peak、FFT、声谱图和相对 dB 映射 | 频率成分随“观测帧时间”变化的可重复显示结果 | 未校准的绝对声压、功率、声压级，以及换能器端真实波形 |

本文把“固件回报的参数”视为代码事实，把“示波器测得的 GPIO 周期”视为电气事实，把“Actions 板载 MIC 经 8765 页面显示的颜色”视为经过测量链路变换后的观测事实。三者不能混为同一个量。
...
`http://127.0.0.1:8765/` 是 Windows 宿主机上的本地分析页面，不是一个独立的“超声波传感器”，也不是直接连接 25T 两端的示波器。当前这次实验的接收链路应写成：
...
ESP32-S3 GPIO4 / 外部驱动 / 25T
             ...
             空气声压与结构耦合
                 ↓
Actions ATS3085S 评估板的板载 MIC 模块
                 ↓
Actions 芯片音频采集链路
增益、滤波、ADC或DSP、采样时序、缓冲
                 ↓
USB UAC PCM（当前会话：16 kHz / 16-bit / mono）
                 ↓
Windows 主机分析程序 / 8765 页面
波形、RMS/Peak、FFT、声谱图、相对 dB
```

因此，页面上看到的不是 `p_air(t)` 的原样复制，而是一个经过板载 MIC、Actions 芯片音频路径、USB 传输、主机缓存和分析算法之后的观测量。可以用一个更贴近当前系统的简化模型表示：
...
其中 `H_board_mic` 表示板载 MIC 模块、安装结构和前端的综合响应；`Q_action` 表示 Actions 芯片内部可能存在的采样、增益、滤波、动态处理、量化和缓冲；`UAC` 表示以 USB 音频格式输出给 Windows 的 PCM 流。当前页面能直接分析的是 `x_uac[n]`，而不是 `y_board(t)` 或芯片内部的原始 ADC 样本。
...
奈奎斯特频率约为 8 kHz，这也对应截图右上角的 `0–8.0 kHz`。因此，当前 8765 页面对导出 PCM 的可视范围是 0–8 kHz；这并不自动证明板载 MIC 元件、Actions 芯片内部 ADC 或模拟前端的完整频响也恰好截止在 8 kHz。
...
一个简单的诊断是把载波依次改成 24、25、26 kHz，并同时记录 GPIO、25T 端和 8765 的导出 PCM。若接收链路中存在可见的直接泄漏分量，16 kHz 导出采样下它大致会向 8、7、6 kHz 移动；如果 3.5 kHz 峰值几乎不动，它就不太可能是载波基波的直接混叠。但这仍不能排除 Actions 板载采集链路在采样前已经完成了非线性解调或滤波；要区分这些情况，必须检查更高带宽的电气/声学测量。
...
| Windows/8765 分析器 | USB 包、主机缓存、轮询、FFT 窗口和 hop | 决定页面显示的时间延迟、平滑程度和频率/时间分辨率 |
...
其中 `w[n]` 是窗函数。当前 8765 分析器使用 1024 点 Hann 窗时，导出 PCM 的频率 bin 间距约为 15.63 Hz，但这不意味着实际频率测量误差必然只有 15.63 Hz；窗函数主瓣宽度、信噪比、峰值插值、Actions 芯片输出的时序稳定性和信号是否稳定都会影响读数。这个参数描述的是主机 FFT，不是板载 MIC 元件本身的“频率分辨率”。
...
| `http://127.0.0.1:8765/` | Windows 宿主机上的 Actions ATS3085S 板载 MIC PCM 分析页：波形、FFT、声谱图和相对 dB |
...
| 8765 的亮暗变化比 GPIO 慢或有延迟 | 声传播、25T 余振、Actions 音频块、USB 缓冲和 FFT 窗口共同造成 | 用 GPIO/25T 示波器触发与原始 PCM 做时间对齐 |
...
如果 status 没变，8765 页面观测到的变化就不能归因于新参数。
...
- 8765 收到的是原始 PCM 还是已经经过芯片端处理的 PCM；
...
5. 8765 声谱图的横轴是主机分析帧时间、纵轴是导出 PCM 的频率、颜色是该帧短时相对能量的可视化；它观察的是 Actions ATS3085S 板载 MIC 链路的输出，不是空气声压原样。
6. 当前会话的 16 kHz UAC PCM/0–8 kHz 页面不能直接观察 25 kHz；这不能替代对板载 MIC、Actions 芯片内部采样和抗混叠路径的硬件/固件确认。
...
3. 8765 页面观察 Actions ATS3085S 板载 MIC 链路中的混叠、非线性和低频可见结果，并把页面参数、PCM 元数据和固件版本一起记录。
